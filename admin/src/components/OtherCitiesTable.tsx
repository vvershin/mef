...
interface OtherCitiesTableProps {
  places: OtherCityPlace[];
  onEdit: (place: OtherCityPlace) => void;
  onDelete: (id: number) => void;
  onToggleFeatured: (id: number) => void;
}

const columnHelper = createColumnHelper<OtherCityPlace>();

export const OtherCitiesTable: React.FC<OtherCitiesTableProps> = ({
  places,
  onEdit,
  onDelete,
  onToggleFeatured,
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('url', {
        header: 'Ссылка',
        cell: info => info.getValue()
          ? <a href={info.getValue()} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">ссылка</a>
          : '-',
      }),
        ...
        cell: props => {
          ...
          return (
            <div className="flex gap-2">
              <button onClick={() => onEdit(place)} className="p-1 hover:bg-gray-100 rounded" title="Редактировать">
                <Edit2 size={16} className="text-blue-600" />
              </button>
              <button
                onClick={() => { if (confirm(`Удалить "${place.title}"?`)) onDelete(place.id); }}
  ...
  return (
    <div className="overflow-x-auto">
          ...
          {table.getHeaderGroups().map(headerGroup => (
            <tr key={headerGroup.id}>
              {headerGroup.headers.map(header => (
                <th key={header.id} className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-b" style={{ width: header.getSize() }}>
      ...
      {places.length === 0 && (
        <div className="text-center py-8 text-gray-500">Нет мест. Создайте первое!</div>
      )}
    </div>
  );
};
